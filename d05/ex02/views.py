...
def populate(request):
	...
	try:
		...
		for data in datas:
			try:
				curr.execute("INSERT INTO ex02_movies (episode_nb, title, opening_crawl, director, producer, release_date) VALUES(%s, %s, %s, %s, %s, %s)", data)
				value.append("<p>OK</p>")
			except Exception as e:
...
def display(request):
	...
	if len(values) == 0:
		return HttpResponse("No data available")
	return render(request, "ex02/display.html", {"values" : values})